...
const ContentManagementPage = () => {
    ...
    const tabs = ["Weekly Practice Videos", "Soft Spark", "Welcome To SundayGirl"];

    // Column headers mapping
    const columns = {
        "Weekly Practice Videos": ["Video Name", "Video Description", "People Joined", "Date Created", "Action"],
        "Soft Spark": ["Video Name", "Status", "People Watched", "Date", "Action"],
        "Welcome To SundayGirl": ["Video Name", "Status", "People Watched", "Date", "Action"],
    };
                ...
                <div className="inline-flex p-1.5 bg-[#F9F9F9] rounded-[30px] mb-8 border border-[#F2F2F2]">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`px-7 py-2.5 rounded-[25px] text-[15px] font-medium transition-all duration-200 ${activeTab === tab
                ...
                <div className="bg-white rounded-[20px] border border-[#F2F2F2] overflow-hidden shadow-[0px_4px_30px_rgba(0,0,0,0.03)] flex flex-col min-h-[500px]">
                    <Table>
                        <TableHeader className="bg-[#F9F9F9]/50 border-b border-[#F2F2F2]">
                            <TableRow className="hover:bg-transparent border-none">
                                {currentColumns.map((col) => (
                                    <TableHead key={col} className="px-8 py-5 text-[15px] font-semibold text-[#24282E] h-auto uppercase tracking-wide">
                                        {col}
                                    </TableHead>
                                ))}
                            </TableRow>
                        ...
                        <TableBody>
                            {currentData.map((row, index) => (
                                <TableRow key={index} className="border-b border-[#F2F2F2] last:border-0 hover:bg-gray-50/30 transition-colors">
                    ...
                    {activeTab !== "Weekly Practice Videos" && (
                        <div className="flex-1 flex flex-col items-center justify-center py-20 bg-white">
                            <h3 className={`${degular.className} text-[28px] text-[#24282E] mb-3`}>
                                {activeTab === "Welcome To SundayGirl" ? "Add New Welcome Video" : `Add New ${activeTab} Video`}
                            </h3>
                            ...
                            <Button
                                onClick={() => {
                                    if (activeTab === "Soft Spark" || activeTab === "Welcome To SundayGirl") {
